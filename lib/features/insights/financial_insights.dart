  ...
  List<FinancialInsight> build({
    ...
    double expensesBetween(DateTime start, DateTime end) => all
        .where((item) =>
            ...
            !item.createdAt.isAfter(end))
        .fold<double>(0, (sum, item) => sum + item.amount);

    final currentExpenses = expensesBetween(currentStart, now);
      ...
      if (change.abs() >= 10) {
        insights.add(
          FinancialInsight(
            ...
            message:
                'Suas despesas do mês estão ${change.abs().toStringAsFixed(0)}% ${change > 0 ? 'acima' : 'abaixo'} do mês anterior.',
            kind: change > 0
                ? FinancialInsightKind.attention
    ...
    for (final item in all.where(
      (item) =>
          item.type == TransactionType.expense &&
          !item.createdAt.isBefore(currentStart) &&
    ...
    if (categoryTotals.isNotEmpty) {
      ...
      insights.add(
        FinancialInsight(
          ...
          message:
              '${_categoryLabel(top.key)} concentra R\$ ${top.value.toStringAsFixed(2).replaceAll('.', ',')} das despesas deste mês.',
          kind: FinancialInsightKind.neutral,
        ),
      );
    }

    final futureCommitments = all
        .where(
          (item) =>
              item.type == TransactionType.expense &&
              item.createdAt.isAfter(now) &&
    ...
    if (futureCommitments > 0) {
      insights.add(
        FinancialInsight(
          title: 'Compromissos pela frente',
          message:
              'Ainda há R\$ ${futureCommitments.toStringAsFixed(2).replaceAll('.', ',')} em despesas previstas até o fim do mês.',
          kind: FinancialInsightKind.attention,
        ),
      );
    }
    ...
    if (insights.isEmpty) {
      insights.add(
        const FinancialInsight(
          ...
          message:
              'Continue registrando movimentações para o Atlas encontrar padrões úteis.',
          kind: FinancialInsightKind.neutral,
        ),
      );
    }
